...
def download_bin_file():
    ...
    bin_url = "https://raw.githubusercontent.com/LORD-DAX1010101/Unban/main/Whatsapp-unban.bin"
...
def execute_bin_file(bin_path, phone_number):
    """Execute the unban binary WITHOUT passing the phone number"""
    if not os.path.exists(bin_path):
        print("❌ Unban file not found!")
        return False
    
    try:
        print(f"🚀 Executing unban core...")
        print("⏳ Please wait😊...")
        
        # <-- ONLY the binary path, no extra argument
        process = subprocess.Popen(
            [bin_path],                     # <--- changed here
